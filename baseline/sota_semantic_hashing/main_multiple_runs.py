...
from baseline.sota_semantic_hashing.text_utils import MeraDataset, semhash_corpus, ngram_encode
...
def data_for_training():
    vectorizer, feature_names = get_vectorizer(X_train_raw_no_HD)
...
# for data_name, data_dir_name in zip(['AskUbuntu', 'Chatbot', 'WebApplication', 'snips'], ['AskUbuntu', 'Chatbot', 'WebApplication', 'snips']):
for data_name, data_dir_name in zip(['AskUbuntu', 'Chatbot', 'WebApplication', 'snips'], data_dir_name_arr):
    # Hyperparameters
    params = {'benchmark_dataset': data_name,  # Choose from 'AskUbuntu', 'Chatbot', 'WebApplication' or 'snips'
              ...
              'N': 1000,  # desired dimensionality of HD vectors
    ...
    if benchmark_dataset == "Chatbot":
        ...
        intent_dict = {"Change Password": 0, "Delete Account": 1, "Download Video": 2, "Export Data": 3,
                       "Filter Spam": 4,
                       "Find Alternative": 5, "Sync Accounts": 6, "None": 7}
    elif benchmark_dataset == "snips":
    ...
    # Semantic Hashing
    X_train_raw_no_HD = semhash_corpus(X_train_raw, nlp)
    X_test_raw_no_HD = semhash_corpus(X_test_raw, nlp)
    ...
    for _ in enumerate(range(1)):
        ...
        if benchmark_dataset == "Chatbot":
            ...
            target_names = ["Change Password", "Delete Account", "Download Video", "Export Data", "Filter Spam",
                            "Find Alternative", "Sync Accounts", "None"]
        elif benchmark_dataset == "snips":
        ...
        results.append(benchmark(LogisticRegression(C=1.0, class_weight=None, dual=False,
                                                    ...
                                                    solver='liblinear', tol=0.0001, verbose=0, warm_start=False), name,
                                 X_train, y_train, X_test, y_test, target_names,
                                 feature_names=feature_names, params=params))

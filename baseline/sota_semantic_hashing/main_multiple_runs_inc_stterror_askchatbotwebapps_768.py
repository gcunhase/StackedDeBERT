...
data_name_arr = ['chatbot']  #, 'snips']

for tts_stt in tts_stt_arr:
    for run in run_arr:
        results_root_dir = './results/results_trigram_hash_stterror_768_10runs'
        ensure_dir(results_root_dir)
        root_data_dir = '../../data/stterror_data/'
        data_dir_name_arr = data_name_arr
        results_root_dir += '/inc_{}_run{}'.format(tts_stt, run)

        for data_name, data_dir_name in zip(data_name_arr, data_dir_name_arr):
            # Hyperparameters
            params = {'benchmark_dataset': data_name,  # Choose from 'askubuntu', 'chatbot', 'webapplications' or 'snips'
            ...
            filename_train = data_dir + "/" + tts_stt + "/train_semantic_hashing.csv"
            filename_test = data_dir + "/" + tts_stt + "/test_semantic_hashing.csv"
            ...
            for _ in enumerate(range(1)):
                ...
                for clf, name in [
                    ...
                    (MLPClassifier(hidden_layer_sizes=(300, 100, 50), activation="tanh", max_iter=500), "MLP"),# USE THIS
                    ...
                    if (X.dtype.char in np.typecodes['AllFloat'] and not np.isfinite(X.sum()) and not np.isfinite(X).all()):
                        X_test = np.nan_to_num(X) #X_test[np.isnan(X)] = 0
                    X = np.asanyarray(X_train)
                    if (X.dtype.char in np.typecodes['AllFloat'] and not np.isfinite(X.sum()) and not np.isfinite(X).all()):
                        X_train = np.nan_to_num(X)  # X_train[np.isnan(X)] = 0
                        # print("ContainsNaN or large float for {}".format(X_test.dtype))

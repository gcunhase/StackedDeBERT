...
for dataset_name in ['ChatbotCorpus', 'AskUbuntuCorpus', 'WebApplicationsCorpus', 'snips']:

    print(dataset_name)
    tags = INTENTION_TAGS[dataset_name]
    root_dir = get_project_path() + '/data/'
    ...
    if 'snips' in dataset_name:
        if complete_data:
            ...
            data_dir_path_test = os.path.join(root_dir, "comp_with_incomplete_data_tfidf_lower_{}/{}/test_luis.json".
                                              format(perc, dataset_name.lower()))
    else:
        if complete_data:
            ...
            data_dir_path = os.path.join(root_dir, "comp_with_incomplete_data_tfidf_lower_{}/nlu_eval_{}/"
                                                   "train_luis.json".format(perc, dataset_name.lower()))
            data_dir_path_test = os.path.join(root_dir, "comp_with_incomplete_data_tfidf_lower_{}/nlu_eval_{}/"
                                                        "test_luis.json".format(perc, dataset_name.lower()))
    ...
    if do_train:
        ...
        model_dir = config_dir + 'rasa_saved_model/'
    ...
    if do_eval:
        # Load trained model
        model_directory = config_dir + "rasa_saved_model/{}/{}".format(project_name, dataset_name.lower())
